...
import flatpick from './global/flatpickr';
import {
  renderHeaderFooter,
  contentFullmetaData,
...
export default class Global extends PageManager {
  onReady() {
    ...
    if (
      mainContent.contains('pages-custom-category-bp-category') ||
      mainContent.contains('pages-custom-category-suits-bp-category')
    ) {
      contentFullmetaData(this.context, (response) => {
        ...
        metadata.contentBlocksCollection.items.forEach((element) => {
          if (
            mainContent.contains('pages-custom-category-bp-category') &&
            element.__typename === 'ReferencedBlockCategoryBanners'
          ) {
          ...
          if (
            mainContent.contains('pages-custom-category-suits-bp-category') &&
            element.__typename === 'ReferencedBlockCategoryBanners' &&
            element.layoutOrientation === 'Image Right'
          ) {
    ...
    if (
      mainContent.contains('pages-custom-category-category-listing') ||
      mainContent.contains('pages-custom-category-suits-category-listing')
    ) {
    ...
    if (
      ...
      productDeatilMetaData(this.context, productId, (response) => {
        ...
        document.getElementById('contentBlocksCollection').innerHTML =
          blocksCollections;
        ...
        if (Object.keys(relatedPro).length > 0) {
          if (relatedPro.thePerfectMatch.length > 0) {
            let perfectmatch = relatedPro.thePerfectMatch.map(
              (item) => item.bc_product_id
            );
            getProducts(contentId, '.thePerfectMatch .prodData', perfectmatch);
          }
          if (relatedPro.youMightAlsoLike.length > 0) {
            let youmaylike = relatedPro.youMightAlsoLike.map(
              (item) => item.bc_product_id
            );
        ...
        document
          .querySelectorAll('.imageWithContentSlider ul')
          .forEach((item) => {
    ...
    if (mainContent.contains('pages-custom-category-category-landing')) {
      let geturl = document
        .getElementById('categoryLanding')
      ...
      getCategorySpecificMetaData(this.context, geturl, (response) => {
        ...
        for (const categoryData of response) {
            ...
            contentfulData?.items?.forEach((element) => {
              if (
                ...
                if (element.collectionBannerImage1) {
                  blockElementFullscreenVideo(
                    'blockElementFullscreenVideo',
                    element.collectionBannerImage1
                  );
                }
                ...
                document.querySelector(
                  ...
                  heading?.replace(lastword, '') !== undefined
              ...
              let blocksCollections = element.contentBlocksCollection.items.map(
                (ele) => {
                  ...
                  if (
                    ele.__typename === 'ReferencedBlockCategoryBanners' &&
                    ele.layoutOrientation === 'Image Right'
                  ) {
          ...
          if (categoryData.key === 'Related Products') {
            ...
            getProducts(
              this.context,
              '.productSlider .productGridSection',
              relatedProductIds,
              5
            );
          }
